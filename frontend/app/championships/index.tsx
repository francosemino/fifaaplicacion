...
export default function ChampionshipsList() {
  ...
  const load = useCallback(async () => {
    const [c, ed, pl] = await Promise.all([api.listChampionships(), api.listEditions(), api.listPlayers()]);
    setItems(c);
    setEditions(ed);
    setPlayers(pl);
  }, []);
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.md, paddingBottom: 40, gap: 10 }}>
        {items.map((c) => (
          <TouchableOpacity
            ...
            testID={`champ-row-${c.id}`}
          >
            <Ionicons name="trophy" size={22} color={colors.gold} />
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>{c.name}</Text>
              <Text style={styles.sub}>
                {edName(c.edition_id)} · {c.status === 'finished' ? `Campeón: ${pName(c.champion_id)}` : 'En curso'}
              </Text>
            </View>
            <Pill
              label={c.status === 'finished' ? 'Final' : 'Activo'}
